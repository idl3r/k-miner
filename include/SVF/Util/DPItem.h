...
public:
    ...
    /// Overloading operator==
    inline bool operator== (const VFPathCond& rhs) const {
        return (context == rhs.getContexts() && path == rhs.getPaths() && edges == rhs.edges);
    }
    /// Overloading operator!=
    inline bool operator!= (const VFPathCond& rhs) const {
        return !(*this==rhs);
 ...
 */
typedef CondVar<ContextCond> CxtVar;
class CxtDPItem : virtual public DPItem {
protected:
    ContextCond context;
...
public:
    ...
    /// Push context
    inline virtual bool pushContext(NodeID cxt) {
