...
void SVFGStat::calculateNodeDegrees(SVFGNode* node, NodeSet& nodeHasIndInEdge, NodeSet& nodeHasIndOutEdge)
{
    ...
    for (; edgeIt != edgeEit; ++edgeIt) {
        ...
        if (isa<CallDirSVFGEdge>(edge)) {
            totalDirCallEdge++;
            const CallDirSVFGEdge * callEdge = dyn_cast<CallDirSVFGEdge>(edge);
            NodeID csId = callEdge->getCallSiteId();
            addCxtToNode(node->getId(), csId);
        } else if (isa<CallIndSVFGEdge>(edge)) {
            totalIndCallEdge++;
            NodeID csId = cast<CallIndSVFGEdge>(edge)->getCallSiteId();
            addCxtToNode(node->getId(), csId);
        } else if (isa<RetDirSVFGEdge>(edge)) {
            totalDirRetEdge++;
            const RetDirSVFGEdge * callEdge = dyn_cast<RetDirSVFGEdge>(edge);
            NodeID csId = callEdge->getCallSiteId();
            addCxtToNode(node->getId(), csId);
        } else if (isa<RetIndSVFGEdge>(edge)) {
            totalIndRetEdge++;
            NodeID csId = cast<RetIndSVFGEdge>(edge)->getCallSiteId();
            addCxtToNode(node->getId(), csId);
        }
    }
...
void
SVFGStat::addCxtToNode(NodeID nodeId, NodeID cxt)
{
    NodeIdSet &inCxts = nodesToContexts[nodeId];
